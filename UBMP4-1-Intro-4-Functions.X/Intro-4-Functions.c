...
Main. Main is always the function that runs first.
...
It is used to show that the function will return a variable to the main code
...
A variable is being set to a value using the = operator, so button_pressed() must be/return a value
...
The value of the variable comes from the "main code". The value is stored in the variable pmwValue
...
Not in any of the functions currently being used. The functions in UBMP.c that are currently being used
have no parameters
...
The button variable gets assigned a value in the main function by setting it equal ( = ) to the output of
button_pressed();
...
The pwm_Value variable gets the value of LED5Brightness. pwm_Value is local to the pwm_LED5() function.
The pwm_LED5 function should be able to use LED5Brightness directly because it is a global variable.
